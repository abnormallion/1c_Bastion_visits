...
var getDbRecords = async data => {
  ...
  var date1 = moment(new Date(`${dateNow} ${data.breakfast.start}`)).toISOString();
  var date2 = moment(new Date(`${dateNow} ${data.breakfast.end}`)).toISOString();
...
var checkAndAddRecord = async (record, i) => {
  ...
  try {
    ...
    if (result.recordsets.length > 0 && result.recordsets[0].length) {
      ...
      var periodinsertion = new Date(moment(record[1], "DD-MM-YYYY HH:mm:ss")).toISOString();
      ...
      var isertion = await request.query("Insert INTO TableTest2 (field1, period, field3, field4, field5, field6) VALUES (@field1, @period, @field3, @field4, @field5, @field6)");
